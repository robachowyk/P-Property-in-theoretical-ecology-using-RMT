...
import normform
import numpy as np
import pmatrix_approximately_correct
import qzmatrix
...
def regising(Ac,Delta):
    ...
    if dg >= 1:
        ...
        dd = 0.1
        return S, dd
    # only true under yet unknown conditions
    if np.array_equal(Delta, np.eye(n)):
        if real_spectral_radius(Ac) > 1:
            S = []
            dd = 10.0
            return S, dd
    ...
    D = []
    #
    if spectral_radius(np.abs(Aci)) < 1:
        dd = 11
        S = []
        return S, dd
    if is_sym_pos_def(Ac.T @ Ac - np.eye(n)):
        dd = 12
        S = []
        return S, dd
    if 1 < np.min(singular_eigvals(Ac)):
        dd = 13
        S = []
        return S, dd
    #
